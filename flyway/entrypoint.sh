...
DATABASE_VERSION=$(PGPASSWORD=password psql -U postgres -h postgres.web3signer-siberiumlab.dappnode -p 5432 -d web3signer -t -A -c "SELECT version FROM database_version WHERE id=1;")
...
if [ "$DATABASE_VERSION" -ge "$LATEST_MIGRATION_VERSION" ]; then
  ...
  flyway -baselineOnMigrate="true" -baselineVersion="${DATABASE_VERSION}" -url=jdbc:postgresql://postgres.web3signer-siberiumlab.dappnode:5432/web3signer -user=postgres -password=password -connectRetries=60 migrate
